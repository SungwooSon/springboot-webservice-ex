...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http
                .csrf().disable()
                .headers().frameOptions().disable() // h2-console 화면을 사용하기 위해 해당 옵션들을 disabled 처리
        .and().authorizeRequests() // URL별 관리를 설정하는 옵션의 시작점.
                                   // authorizeRequests가 선언되어야만 antMatchers 옵션을 사용할 수 있다.
        .antMatchers("/css/**", "/images/**", "/js/**", "/h2-console/**", "/").permitAll()
        .antMatchers("/api/v1/**").hasRole(Role.USER.name()) // antMatchers : 권한관리 대상을 지정하는 옵션.
                                                                         // URL, HTTP 메소드별로 관리가 가능.
                                                                         // "/" 등 지정된 URL들은 permitAll() 옵션을 통해 전체 열람 권한을 준다.
                                                                         // "/api/v1/**" 주소를 가진 API는 USER 권한을 가진 사람만 열람 가능하도록 함.
        .anyRequest().authenticated() // 설정된 값들 이외 나머지 URL들을 나타냄.
                                      // 여기서는 authenticates()를 추가하여 나머지 URL들은 모두 인증된 사용자들에게만 허용.
                                      // 인증된 사용자 즉, 로그인한 사용자들을 이야기함.
        .and().logout().logoutSuccessUrl("/") //로그아웃 기능에 대한 여러 설정의 진입점. 로그아웃시 / 주소로 이동
